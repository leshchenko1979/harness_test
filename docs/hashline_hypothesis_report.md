...
| **Collision read format (H3)**   | **Rejected** — do not ship `                            | ` default                                                                                                                                                                                                                                                                            |
...
| `hashline_edit`       | 2 (+ 4 per edit item)         | `[hashline_edit in hashline.py](../experiments/tooling/opencrabs/hashline_edit.py)`                                                               |
...
| ID     | Hypothesis                                                                     | Isolated change                                                               | Success criterion                                                                      | Primary cases                                                                         |
| ------ | ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| **H1** | Fixing docs to describe `HASH                                                  | content`(not`LINE#ID`) improves success                                       | `[opencrabs_h1_docs.yaml](../experiments/tool_sets/opencrabs_h1_docs.yaml)`            | Pass rate ↑ vs original on indent/collision-related cases                             |
| **H2** | **Two edit tools → one** fuzzy line-block `str_replace` improves outcomes      | `[opencrabs_h2_fuzzy.yaml](../experiments/tool_sets/opencrabs_h2_fuzzy.yaml)` | Pass rate ≥ original on ambiguous/rename/large cases                                   | [corpus §5](#test-corpus): ambiguous*, rename large, docstring large                  |
| **H3** | Showing collisions as `                                                        | {line}`on read beats`COLLISION                                                | {line}`                                                                                | `[opencrabs_h3_collision.yaml](../experiments/tool_sets/opencrabs_h3_collision.yaml)` |
| **H4** | Original OpenCrabs toolset worse than simplified reference on indented py/yaml | original vs `[baseline.yaml](../experiments/tool_sets/baseline.yaml)`         | Lower pass for original on [H4 trap cases](#h4-reference-simplified-tool-set-baseline) | [corpus §5](#test-corpus): whitespace trap*, large py/yaml traps                      |
...
**H3 — Empty-hash collision display:** Rejected — regressed `[whitespace_trap](../experiments/cases/whitespace_trap.yaml)` and `[indent_collision](../experiments/cases/indent_collision.yaml)` (27 turns on the latter). Worst efficiency in [§7](#7-results). Do not default to `\|` on read.
